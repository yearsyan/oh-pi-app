...
- 持久化任务支持标准五段式 Cron、固定间隔、指定时间单次执行和免 Token 的密钥 HTTP 触发器（可按毫秒参数延迟执行）；每次执行创建独立 session，并可指定工作空间、模型、思考强度、任务专属 Skills（含 `--no-skills`）和初始化 Prompt。每个任务可分页查看全部关联 session；普通会话列表可按来源隐藏任务 session，服务端默认在闲置 7 天后自动清理。
...
脚本会按系统/架构下载 Release 二进制并校验 SHA-256，生成 token 与 `~/.config/oh-pi-app/config.json`，注册用户级服务（macOS LaunchAgent / Linux systemd user，无 systemd 时回退 nohup）并启动，最后通过 `/healthz` 验证。网关只监听 `127.0.0.1:18080`，不涉及 TLS。未检测到 `pi` 时脚本会自动安装 Node.js 22 与 `pi`（无 sudo、不修改 shell 启动文件）；Node/npm 会先对官方源与 npmmirror 测速，仅在镜像版本一致且明显更快时使用，任一下载失败都会尝试另一个源。设置 `OHPI_NO_PI_INSTALL=1` 可跳过自动安装。重复执行同一命令会升级二进制并重启服务，token、配置与 session 全部保留。子命令与环境变量见[部署与配置](docs/deployment.md#快速安装脚本macos--linux)。
