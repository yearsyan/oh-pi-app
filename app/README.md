...
- **工作空间 / 会话详情**：从网关 HTTP API 读取服务端权威工作空间及分页会话，支持新建、恢复
  ...
  列表按技术栈显示图标并支持继续加载；长按工作空间可编辑、归档或删除，删除工作空间时
  保留全部会话和目录文件，同目录重新添加后自动恢复。工作空间设置支持多组 Skills 与
  Extensions 路径，以及关闭其他资源的自动发现；已归档工作空间也可在设置中删除。
- **会话内对话**：流式渲染助手消息（Markdown：标题/列表/引用/代码块/行内样式），
  运行中可随时中止（abort）或引导（steer）。
- **思考过程**：可折叠的思考卡片，流式更新。
- **定时任务**：独立列表页和独立创建/编辑页，可通过星期与时间表单配置周期计划，也支持固定间隔、单次时间或带 32 位私密 event key 的 HTTP JSON 触发器，
  并选择工作空间、模型、思考强度、任务专属 Skills 路径、是否排除自动发现的 Skills 和初始化 Prompt；支持暂停、立即执行和删除，长按任务可进入独立页面查看全部关联 session。设置中可从普通会话列表隐藏
  定时任务 session，并配置服务端按最近活跃时间自动清理的保留天数（默认 7 天）。
