...
internal data class ProcessSummary(
    ...
    /** Distinct file targets written/edited; names the summary when there is exactly one. */
    val writtenFiles: List<String> = emptyList(),
    /** Distinct file targets read; names the summary when there is exactly one. */
...
internal fun summarizeProcessDetails(details: List<AssistantProcessDetail>): ProcessSummary {
    ...
    return ProcessSummary(
        ...
        writtenFiles = writtenTargets.toList(),
        readFiles = readTargets.toList(),
    )
}
